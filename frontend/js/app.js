...
const HORARIOS_POR_TURNO = {
  1: ["05:00", "06:00", "07:00", "08:00", "09:00", "10:00", "11:00", "12:00", "13:00"], //[cite: 1]
...
function selecionarMaquina(numMaquina) {
  ...
  document.getElementById("badgeDetalheMaquina").innerText = `Cavidades: ${info.cavidades} | Ciclo: ${info.ciclo}s`;
...
function renderizarTabela() {
  ...
  horas.forEach((hora) => {
    const salvo = registrosState[maquinaAtiva][hora] || { prod: "", inicio: "", retomada: "", motivo: "" };
...
function salvarValor(hora, campo, valor) {
  if (!registrosState[maquinaAtiva][hora]) {
    registrosState[maquinaAtiva][hora] = { prod: "", inicio: "", retomada: "", motivo: "" };
  }
...
async function confirmarFechamento() {
  ...
  try {
    const res = await fetch("http://localhost:8000/turnos/fechamento", {
      method: "POST",
